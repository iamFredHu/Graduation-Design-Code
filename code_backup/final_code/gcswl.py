...
def my_method(m, nodelist, D, Data, v, t_max, e, wmax, supply):
    ...
    for k in range(m):
        ...
        print("path[%d]:" % k)
        print(path[k])
        print("path[%d]Path Time:%fs = %fmin" % (k, T[k][-1], T[k][-1] / 60))
        print("--------------------------------------------")
    ...
    print("Re:%f" % effective_coverage_rate)  # 即在POI规定时间内访问（允许超20%），超过时间就是无效访问
    ...
    print("R:%f  Re:%f  Ro:%f" % (coverage_rate, effective_coverage_rate, onTime_rate))
    print(path)
...
if __name__ == '__main__':
    ...
    line1=[]
    for i in path[1]:
        line1.append(coordinate_list[i])
    print(line1)
